...
import { ArrowLeft, Mail, Lock, User, Eye, EyeOff, TrendingUp, Shield, Zap, CheckCircle2 } from 'lucide-react';
...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setUser(data.user);
      localStorage.setItem('santibet_token', data.access_token);
      toast.success('Account created! Welcome to Santibet.');
  ...
  return (
    <div className="flex min-h-screen">
        ...
        <div className="flex flex-1 items-center justify-center px-6 pb-20">
          <div className="w-full max-w-sm space-y-6">
            <div className="space-y-2 text-center">
              ...
              <p className="text-sm text-muted-foreground">Start trading on real-world events</p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="name" type="text" placeholder="Alex Chen" className="pl-9" required value={name} onChange={(e) => setName(e.target.value)} />
                </div>
              </div>
              <div className="space-y-2">
                ...
                {password.length > 0 && (
                  <div className="flex flex-wrap gap-2 pt-1">
                    {passwordChecks.map((c) => (
                      <span key={c.label} className={`flex items-center gap-1 text-xs ${c.valid ? 'text-success' : 'text-muted-foreground'}`}>
              ...
              <div className="flex items-start gap-2">
                ...
                <Label htmlFor="terms" className="text-sm font-normal leading-snug">I agree to the <Link href="#" className="text-primary hover:underline">Terms of Service</Link> and <Link href="#" className="text-primary hover:underline">Privacy Policy</Link></Label>
              </div>
              <Button type="submit" className="w-full" disabled={loading}>{loading ? 'Creating account...' : 'Create Account'}</Button>
            </form>
            <div className="relative"><div className="absolute inset-0 flex items-center"><span className="w-full border-t border-border" /></div><div className="relative flex justify-center text-xs uppercase"><span className="bg-background px-2 text-muted-foreground">Or sign up with</span></div></div>
      ...
      <div className="relative hidden w-1/2 flex-col justify-between overflow-hidden bg-gradient-to-br from-chart-2/10 via-card to-primary/10 p-10 lg:flex">
        <div className="absolute inset-0 bg-grid opacity-20" />
        <div className="absolute bottom-0 left-0 h-64 w-64 rounded-full bg-chart-2/20 blur-[100px]" />
        <div className="relative flex justify-end"><Logo /></div>
        <div className="relative space-y-6">
          <h2 className="font-display text-3xl font-bold leading-tight">Start predicting. Start earning.</h2>
          <p className="text-muted-foreground">Create a free account in seconds. Get AI-powered market recommendations and trade on 300+ active prediction markets.</p>
